...
class LoginService(val userRepository: UserRepository) {
    ...
    fun login(loginDto: LoginDto): ResponseEntity<*> {
        ...
        val x = userRepository.findPasswordByEmail(loginDto.email)

        val y = BCryptPasswordEncoder()


        val user = userRepository.findUserByEmail(loginDto.email)

        println(user)

        //println(loginDto.username)
        //println(loginDto.password)                                                
                                       
        return if (user != null && y.matches(loginDto.password,x)) {  //&& temp==1
