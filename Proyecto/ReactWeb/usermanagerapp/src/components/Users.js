...
const baseUrl = "http://localhost:64499/api/Users";
...
export function List()
{
  ...
  const postUser=async() => {
    ...
    await axios.post(baseUrl, currentUser, config)
    .then (response=>{
      getUsers();
  ...
  const putUser = async() => {
    await axios.put(baseUrl+"/"+ currentUser.id, currentUser, config)
    .then (response=>{
  ...
  const deleteUser = async() => {
    await axios.delete(baseUrl+"/"+ currentUser.id, config)
    .then (()=>{
